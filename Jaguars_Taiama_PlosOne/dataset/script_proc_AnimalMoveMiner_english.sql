...
--  *****************************  DELETE AUXILIAR TABLES  ******************************
      ...
      DROP TABLE IF EXISTS TB_Rule_Animal_Animal_Soil;
	  DROP TABLE IF EXISTS TB_Rule_Animal_State_Cover_Annual;
	  DROP TABLE IF EXISTS TB_Rule_Animal_Animal_Filhote;
...
--  *********************  CREATE TABLE TO STORE THE LAND COVER  ********************             
...
/*###################################################################################################################
### 
### *********************************     IDENTIFY THE LAND COVER TO EACH  MOVEMENT DATA ****************************
### 
#####################################################################################################################*/
	              
DELIMITER $$
DROP PROCEDURE if exists preparaDadosCobertura$$
CREATE PROCEDURE `preparaDadosCobertura`()
BEGIN

DELETE FROM TB_RST_MovAnimal_Estado_Copia;

INSERT INTO TB_RST_MovAnimal_Estado_Copia
SELECT individual, A.lat, B.lon, timestamp, res, estado, A.area, C.nm_class_english
from tb_cobertura_taiama  A INNER JOIN TB_RST_MovAnimal_Estado B ON (A.lat = B.lat and A.lon  = B.lon) 
inner join classif_cobertura_solo C ON (A.area = id_class);
...
/*###################################################################################################################
### 
### *********************************     IDENTIFY THE PERIOD WITH CUBS JAGUAR (FILHOTE)  ****************************
### 
#####################################################################################################################*/
	              
DELIMITER $$
DROP PROCEDURE if exists indPeriodoFilhote$$
CREATE PROCEDURE `indPeriodoFilhote`()
BEGIN

alter table TB_RST_MovAnimal_Estado add status_filho varchar(5);

update TB_RST_MovAnimal_Estado
set status_filho =  (CASE WHEN timestamp between '2019-09-20 00:00:00' and '2019-10-15 23:59:59' THEN 'YF' 
					WHEN timestamp >= '2019-10-16 00:00:00' THEN 'NF'	
					WHEN timestamp <= '2019-09-19 23:59:59' THEN 'NO' END);
