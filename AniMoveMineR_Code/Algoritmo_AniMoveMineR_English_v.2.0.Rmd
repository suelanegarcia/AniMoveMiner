--- 
title: "AniMoveMineR: Análise Exploratória da Interação entre Animais e o Meio (By Suelane Garcia)"
...
params:
  ...
  animal:
    input: select
    label: "Analisa a interação entre animais"
    value: N
    choices: ["S","N"]
  
  filho:
    input: select
    label: "Analisa animal com filhote"
    value: N
    choices: ["S","N"]
  ...
  soil:
    input: select
    label: "Analisa Cobertura do Solo"
    value: N
    choices: ["S","N"]
...
if (params$filho == "S"){
   conn <- dbConnect(MySQL(),user="root", db="weka",host="localhost")
  data <- dbSendQuery(conn, "call indPeriodoFilhote();")
...
if (params$animal == 'S') {
  ...
  dataAssoc <- dbSendQuery(connMove,"select  year(date1) year, month(date1) mon, 
                                      ind1 Animal1, ind2 Animal2, AVG(dist) Dist_Media 
                                     from TB_Distancia_Animal where ind1 <> ind2  group by  ind1, ind2" )
  ...
  for (b in 1:t){
     ...
     labs(title="", subtitle= "", caption= paste("Year: ", dsa$year, sep="")) +
     ...
     plot(p)
        ...
        sql <- paste("select ind1 Animal1, ind2 Animal2, date(date1) date,  date1, concat(year(date1), 
                       ...
                       " group by yearmon, ind1, ind2, date(date1) order by yearmon, 
                     ind1, ind2, date(date1)", sep = "")
...
```{r, include="FALSE"}
#### *******************************************************************************************
#### *** C. IDENTIFIES CORRELATION BETWEEN NEIGHBORS ANIMALS  (ANIMAL x ANIMAL) - MONTH/YEAR ***
#### *******************************************************************************************
if (params$animal == 'S') {
connAssoc <- dbConnect(MySQL(),user="root", db="weka",host="localhost")
sql <- paste("select  ind1, ind2, concat(year(date1), month(date1)) as yearmon, 
              year(date1) year, month(date1) mon 
...
if (length(Rule) > 0) {
    ...
    for (a in 1:length(dsYear1)){
       ...
       x <- ggplot(dsa, aes(dsa$Animal1, dsa$Animal2,  vjust=0.1, 
            ...
            geom_tile(aes(fill = dsa$Phi), colour = "white") + 
            scale_fill_gradient(low = "white", high = "steelblue", limits=c(0,100)) + 
            xlab(" Animal 1") + ylab("Animal 2") + labs(fill = "Phi(%)")  +  
            geom_text_repel(color = "black", size = 3) +
            theme(axis.text.x = element_text(angle=85, vjust=0.6)) + 
            labs(title="Correlation level between animals by Month and Year", 
            subtitle= paste("minSup: ", params$minSupConf[a], " | ", "minConf: ", 
                            params$minSupConf[a], sep=""), 
            caption= paste("Year: ",dsa$year, sep="")) + facet_wrap(~dsa$Month, ncol= 2)
       plot(x)
       ...
       z <- ggplot(dsa, aes(dsa$Animal1, dsa$Animal2, hjust=1, 
         ...
         scale_fill_gradient(low = "white", high = "steelblue",  
                             limits=c(0,100)) + xlab(" Animal 1") + ylab("Animal 2") + 
      ...
      #    quality(rules)$phi  <- interestMeasure(rules, measure='phi', significance=T, transDados)
          ruledf = data.frame(id = ym, year= year, mon = mon,
                              lhs = labels(lhs(rules)), rhs = labels(rhs(rules)),
                               ...
                               supB = support(rhs(rules), transDados), 
                               supEsp = (support(lhs(rules), transDados) * support(rhs(rules), transDados)), 
                               test =  paste("mSup=", params$minSupConf[b], "/mConf=", 
                                             params$minSupConf[b], sep = ""),
...
if (length(Rule) > 0) {
    ...
    for (b in 1:t){
       ...
       geom_bar(position="dodge", stat="identity",  width = 0.4) +
                theme(axis.text.x = element_text(angle=85, vjust=0.6)) + 
         labs(title="Correlation level between animals", 
                subtitle= "for different minimum support and confidence values",
                 caption= paste("Year: ", dsa$year, sep="")) +   
                xlab("Animals") + ylab("Phi") + scale_fill_discrete(name = "Minimum Support/Confidence") 
       plot(p)
    ...
    for (a in 1:length(dsYear1)){
       ...
       x <- ggplot(dsa, aes(dsa$Animal1, dsa$Animal2,  vjust=0.1, 
            ...
            geom_text_repel(color = "black", size = 3) +
            theme(axis.text.x = element_text(angle=85, vjust=0.6)) + 
            labs(title="Correlation level between animals per year", 
            subtitle= paste("minSup: ", params$minSupConf[a], " | ", "minConf: ", 
                            params$minSupConf[a], sep=""), 
            caption= paste("Year: ",dsa$year, sep=""))  
...
shapename <- read_sf('C:/Users/suela/Downloads/shape_taiama/shape_aquida_2018.shp')

dse <- as.data.frame(shapename$DN)
ds  <- filter (dse, dse$`shapename$DN`!= 0)
pal <- leaflet::colorFactor(palette = c("9" = "#00FFFFFF", "3" = "#228B22", 
                                        "4" = "#DAA520",    "11" = "#808000", 
                                        "12" = "#D2691E",  "15"= "#FFD700",
                                        "19"= "#836FFF",  "20"= "#8B4513",
                                        "24" = "#F4A460" , "25"= "#FF00FF", "33" = "#0000FF"),  domain = shapename$DN)
...
```{r}
#### ******************************************************************************************
#### E.IDENTIFY THE CORRELATION BETWEEN ANIMALS (ANIMALS AND LAND COVER)                      *  
#### ******************************************************************************************
if (params$soil == 'S') {
connAssoc <- dbConnect(MySQL(),user="root", db="weka",host="localhost")
sql <- "SELECT estado, nm_cobertura, individual, concat(year(timestamp), individual) as yearmon,
        year(timestamp) year, month(timestamp) mon  
        FROM TB_RST_MovAnimal_Estado_CopiA B"
...
```{r}
#### ****************************************************************************************
#### *** IDENTIFIES CORRELATION BETWEEN ANIMALS AND CUBS (Filhote) - MONTH/YEAR ***
#### ****************************************************************************************
if (params$filho == "S") {
connAssoc <- dbConnect(MySQL(),user="root", db="weka",host="localhost")
sql <- "SELECT estado, status_filho, individual, concat(year(timestamp), 
        month(timestamp), individual) as yearmon, year(timestamp) year, month(timestamp) mon  
...
if (length(Rule) > 0) {
    name_file = "TB_Rule_Animal_Animal_Filhote"
    dbWriteTable(connAssoc, value = Rule, name = name_file, append = T) 

    SQLRule = " select test, cast(mon as integer) mon, lhs, rhs, supA, supB,
    cast((IFNULL(CAST(support AS DECIMAL(6,2)), 1)  * 100) as decimal(3))  'Sup', 
    cast((IFNULL(CAST(confidence AS DECIMAL(6,2)), 1)  * 100) as decimal(3)) 'Conf',     
  	(CASE WHEN  support = 1 and confidence = 1 and phi is null 
        THEN cast((IFNULL(CAST(phi AS DECIMAL(6,2)), 1)  * 100) as decimal(3))
        Else  cast((CAST(phi AS DECIMAL(6,2))  * 100) as decimal(2)) END) Phi,     
        cast(lift as decimal(3)) lift, count as 'Occur'
    From TB_Rule_Animal_Animal_Filhote  where lhs not like '%{}%' and lhs not like '{status%'
	  order by test, cast(mon as integer), phi desc;"
